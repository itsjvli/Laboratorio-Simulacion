.data
    ...
    msg_result:  .asciiz "El numero menor es: "
...
.text
    ...
    read_loop:
        ...
        lw $t5, 0($t4)       # Inicializar el mínimo con el primer número
        addi $t4, $t4, 4     # Mover a la siguiente posición
        li $t6, 1            # Índice para comparar el resto de los números
        
    find_min:
        lw $t7, 0($t4)       # Cargar el siguiente número
        ble $t5, $t7, skip   # Si $t5 (mínimo) es menor o igual, saltar
        move $t5, $t7        # Actualizar mínimo
    skip:
        addi $t4, $t4, 4     # Mover a la siguiente posición
        ...
        li $v0, 1
        move $a0, $t5        # Mostrar el número menor
        syscall
